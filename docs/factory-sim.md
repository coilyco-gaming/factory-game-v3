...
- `factory_sim` - deterministic inventory, typed dispatch protocol, hauler, factory, and tick stepping.
...
The CLI prints JSON lines. Each line contains the tick number, the current source, hauler, and factory snapshots, the typed dispatch protocol state, and the events emitted during that tick.
