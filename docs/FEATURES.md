...
- **Bevy/Wasm simulation viewer** - `crates/factory_shell` now hosts the deterministic simulation behind a native and browser-capable Bevy projection. It renders topology nodes, haulers, inventories, dispatch state, craft progress, run metrics, and current-tick events with play, pause, step, reset, and speed controls. Bevy consumes immutable snapshots and owns no simulation rules. See [factory-viewer.md](factory-viewer.md) and [factory-shell.md](factory-shell.md).
