...
- **Bevy/Wasm app shell** - `crates/factory_shell` is the deployable web shell: a minimal Bevy scene that builds native and for `wasm32-unknown-unknown`, packaged by trunk and served by the repo-root nginx Dockerfile on the deploy repo's static-site pattern. No sim coupling - see [docs/factory-shell.md](factory-shell.md).
