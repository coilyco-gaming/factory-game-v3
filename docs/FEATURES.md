...
- **Bevy/Wasm world viewer** - `factory_shell` projects immutable simulation snapshots natively and in browsers. It renders topology, movement, inventories, logistics, radar claims, deployment, production, power, metrics, and matching local art across four selectable 50x50 worlds. A full-width status bar and bottom-right control panel are the only screen-space UI surfaces. The camera starts fitted to the full extent, cannot zoom farther out, and retains an approximately 10x10-cell close view. Held-key navigation and focus mode keep each world inspectable. See [factory-viewer.md](factory-viewer.md), [factory-shell.md](factory-shell.md), and [v3-worlds.md](v3-worlds.md).
