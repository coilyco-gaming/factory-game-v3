...
- **Current test surface** - `tests.csproj` holds the existing xUnit coverage for the C# baseline. Run it through `ward exec test`, which restores first and then runs the suite with bounded phase timeouts.
