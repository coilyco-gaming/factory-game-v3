...
- **Bevy/Wasm simulation viewer** - `crates/factory_shell` hosts the deterministic simulation behind a native and browser-capable Bevy projection. It renders topology nodes, smoothly moving haulers, inventories, dispatch state, craft progress, run metrics, and current-tick events. The continuous showcase cycles through all three starter scenarios, with controls for play, pause, step, reset, speed, scenario advance, and cycle locking. Bevy consumes immutable snapshots and owns no simulation rules. See [factory-viewer.md](factory-viewer.md) and [factory-shell.md](factory-shell.md).
