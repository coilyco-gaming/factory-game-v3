...
- **Local runtime factory art** - seven accepted 100px RGBA sprites live under `crates/factory_shell/assets/factory/` in ordinary Git. A typed Bevy resource overlays them on matching ground, road, truck, iron-deposit, foundry, and cargo identities while retaining colored fallbacks, interpolation, labels, gauges, route effects, and overview performance. See [factory-art.md](factory-art.md).
...
- **Bevy/Wasm world viewer** - `factory_shell` projects immutable simulation snapshots natively and in browsers. It renders topology, movement, inventories, logistics, radar claims, deployment, production, power, metrics, activity, and matching local art. The UI roster contains only complete world simulations, while focused fixtures remain headless regression scenarios. The camera starts fitted to the full 50x50 extent, cannot zoom farther out, and retains an approximately 10x10-cell close view. Accelerated grid panning, a clickable deck, and focus mode keep the world inspectable. See [factory-viewer.md](factory-viewer.md) and [factory-shell.md](factory-shell.md).
