...
- **Headless play surface** - `factory_cli play` drives the compact loop over a line-oriented JSON protocol with typed edit rejections, a tick budget, and save round-tripping, so the game is scriptable. See [headless-play.md](headless-play.md).
