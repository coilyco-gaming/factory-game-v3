...
image_name="coilyco-gaming/factory-game-v3"
...
case "${sha}" in
  *[!0-9a-f]*|"")
    echo "factory-game-v3 source sha is not a lowercase hexadecimal commit id." >&2
    exit 1
    ;;
esac
if [ "${#sha}" -ne 40 ]; then
  echo "factory-game-v3 source sha must be a full 40-character commit id." >&2
  exit 1
fi
